...
document.querySelector("#add").onclick = function () {
  ...
  if (
    ...
    if (input[0].value.length == 0) {
      ...
      //ensure date and time is not empty
      alert("Add Time to do your task ");
    }
  } else {
    ...
    document.querySelector("#newTask").innerHTML = `<h1>All the Tasks</h1>`;
    document.querySelector("#tasks").innerHTML += `
        <div class="task" id=task${counter}>
                <input id=${counter} class="checkbox" type="checkbox" name="checkbox" onclick="toggleCheck(this.id)" /> Done
                <span id="taskname">
                    <div>
                        <div>
    ...
    for (var i = 0; i < edit.length; i++) {
      edit[i].onclick = function () {
        //when specific edit button is clicked
        ...
        document.getElementById("update").onclick = function () {
          ...
          if (
            ...
            for (var i = 0; i < edittext.length - 1; i++) {
              //for each input update corresponding <p> html element
